...
export async function getTagsForLevel(level: string): Promise<string[]> {
    const lvl = level.toLowerCase();

    const sentences =
        lvl === "all" ? await loadAllLevels() : await loadOneLevel(lvl);
...
export async function showSentences(level?: string, tag?: string): Promise<void> {
    ...
    if (tag) {
        ...
        sentences = sentences.filter(
            (s) => Array.isArray(s.tags) && s.tags.some((t) => t.toLowerCase() === tagLower),
        );
    }
